...
// Assignment 4
// randomtestcard1.c
// This program implements a random tester for the smithy card
...
// This is the actual test suite that takes a random input and tests smithy and its related functions
void smithyTest(struct gameState *currentGameTest, int playerCount){
    ...
    int cardEffectTestResult = cardEffect(smithy, 0, 0, 0, currentGameTest, 0, &coin_bonus);
    
    // Manually call the functions performed by the smithy card
    int drawCardTest1 = drawCard(playerCount, &copyGameTest);
    int drawCardTest2 = drawCard(playerCount, &copyGameTest);
    int drawCardTest3 = drawCard(playerCount, &copyGameTest);
    ...
    if(drawCardTest2 == -1 && copyGameTest.deckCount[playerCount] != 0){
        drawFailures += 1;
        currentFail += 1;
    }
    
    if(drawCardTest3 == -1 && copyGameTest.deckCount[playerCount] != 0){
        drawFailures += 1;
        currentFail += 1;
    }
...
int main(){
    ...
    printf("This program will test the Smithy card in dominion.c. Please stand by for results:\n");
    ...
    for(i = 0; i < numberOfTests; i++){
        ...
        smithyTest(&currentGameTest, playerCount);
    }
    
    // Output the results
    printf("The following are the results from the test:\n");
    
    if(drawFailures + discardFailures + handCountFailures + cardEffectFailures + deckCountFailures > 0){
        printf("There were %d successful test runs.\nThere were %d failed test runs.\n", numberOfTests - failedTestRuns, failedTestRuns);
        printf("There were %d failed individual tests!\n", drawFailures + discardFailures + handCountFailures + cardEffectFailures + deckCountFailures);
