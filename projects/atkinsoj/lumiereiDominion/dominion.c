...
// Below are the functions that were refactored for assignment 2. 4 of the 5 functions include minor bugs
// as directed by the assignment 2 instructions. For more info on these bugs, please see the PDF.
...
int remodel_function(struct gameState *state, int handPos, int choice1, int choice2){
    int i;
    int j;
    int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (nextPlayer > (state->numPlayers - 1)){
    nextPlayer = 0;
        ...
        {
            case adventurer:
                result = adventurer_function(state, handPos);
                return result;
            
            case council_room:
                result = council_room_function(state, handPos);
                return result;
            
            case smithy:
                result = smithy_function(state, handPos);
                return result;
            
            case remodel:
                result = remodel_function(state, handPos, choice1, choice2);
                return result;
                
            case village:
                result = village_function(state, handPos);
                return result;
